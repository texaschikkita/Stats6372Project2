...
# model comparison on AIC and BIC:print(paste("AIC Model 1:", aic_model1, "vs. AIC Model 2:", aic_model2))
...
# Clinical or Practical Significance: This would be more of a discussion based on the predictors used in each model. Can we discuss as a team? 
...
# performance on validation? or performance on testing?  canyone wanna jump onin?  maybe ill add some fun new variables like dead people and seehwo that effects the accuracy.  gimme a thumbs up or down!!
...
# RAFIA (MUCH HAS BEEN REWORKED. HOWEVER WE STILL NEED TO GO OVER IT AMD MAKE SURE THAT WE ARE ALL BUILDING FROM THE SAME MODEL IN THE EDA.  THE IDEA IS TO IMPROVE UPON THE MODEL EMPLOYING AND TESTING WITH VARIOUS TECHNIQUES,.  Can you zoom tonight or tomorrow or the day after or anytime?)
# 
# *** 23 histograms seems overkill and bnot extrenely informative.  Sorry tro be critical - but can we use some more intepretative and informative plots?  I have sent yall a few suggestions with examples in the revised r scriupts/ r notebooks and, htmls and pdfs but havent heard back. :(  )
...
# CALEB (i modified alot but more still needs tro be corrected. the numbers are off.  can we review together tonight?)
...
# Balance Dataset with smote using Rose ROSE PACKAGE OR SMOTE FAMILY NEDDS TO BE USED AND SMOTEFAMILY IS MEH. 
...
# Random Forest
## Ensure 'Fracture' is a factor with appropriate levels this part is annoying me   changing back over to r naotebook i cant w r markdown. annoying,  
rf_data$Fracture <- factor(rf_data$Fracture, levels = c(0, 1))
...
# make sure  'Fracture' is currently a numeric 0 and 1
...
# Assuming i've already split data and trained models, now get probabilities: i mean i keep splitting it but sometimes r likes to play games w me and error out at random code chunks bc the variables have changed and it doesnt update.  creating a new library to fix this and publishing soon  n the meantime r bnotebook for the win
